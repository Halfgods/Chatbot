...
def load_rag_data(file_path):
    """
    RAG data ko response.json file se load karta hai aur cache karta hai. 
    Deployment ke liye isse ek hi baar load kiya jayega.
    """
    try:
        # Assuming response.json is in the same directory as the script
        with open(file_path, 'r', encoding='utf-8') as f:
            # Assuming the JSON structure is a list of {"question": "...", "answer": "..."} objects
            data = json.load(f)
            
            # Data ko jaldi lookup ke liye dictionary mein badalna (optional, but good practice)
            # Yahan hum har question ko as a key use kar rahe hain
            rag_map = {item.get("question", "").strip().lower(): item.get("answer", "") 
                       for item in data if isinstance(item, dict) and item.get("question")}
            
            return rag_map
            
    except FileNotFoundError:
        st.warning(f"⚠️ Warning: RAG file '{file_path}' not found. Chatbot will use only Gemini API.")
        return {}
...
def main():
    ...
    try:
        ...
        if prompt := st.chat_input("Type your message here...", key="chat_input"):
            ...
            if rag_data and user_query_clean in rag_data:
                # 1. RAG Hit: Agar exact match milta hai
                ...
                # 2. RAG Miss: Agar match nahi milta, toh Gemini API call karo
                with st.chat_message("assistant", avatar="🤖"):
